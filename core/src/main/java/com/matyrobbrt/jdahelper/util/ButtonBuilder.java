...
public class ButtonBuilder implements Supplier<Button> {
    ...
    /**
     * Sets the button's ID, if the {@link #style style} is not {@linkplain ButtonStyle#LINK}.
     *
     * @param id the button id
     * @return the builder instance
     * @throws UnsupportedOperationException if the {@link #style style} is {@linkplain ButtonStyle#LINK}.
     */
    public ButtonBuilder id(String id) {
        if (style == ButtonStyle.LINK) {
            throw new UnsupportedOperationException("Cannot set a link button's ID!");
        }
        return idOrUrl(id);
    }

    /**
     * Sets the button's url, if the {@link #style style} is {@linkplain ButtonStyle#LINK}.
     *
     * @param url the button url
     * @return the builder instance
     * @throws UnsupportedOperationException if the {@link #style style} is not {@linkplain ButtonStyle#LINK}.
     */
    public ButtonBuilder url(String url) {
        if (style != ButtonStyle.LINK) {
            throw new UnsupportedOperationException("Can only set URLs on link buttons!");
        }
        return idOrUrl(url);
    }
